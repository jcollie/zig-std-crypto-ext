...
//! linear in how far the type overshoots the key.
//!
//! An RSA implementation that supports 4096-bit keys instantiates
//! `Modulus(4096)` for all of them, so every 2048-bit key paid twice. On a
//! 2026 x86-64 laptop in ReleaseFast, one 2048-bit private exponentiation:
//!
//! | | std | here |
//! | --- | --- | --- |
...
//!
//! Constant time is unaffected. The new length depends on the modulus, which
//! is public; it does not depend on the exponent, which is not.
//! `powPublic` beside it already trims this way and goes further, stripping
//! leading zero *bits* -- which the secret path must not do, because that
...
pub fn Modulus(comptime max_bits: comptime_int) type {
    return struct {
        ...
        pub fn pow(self: Self, x: Fe, e: Fe) (NullExponentError || RepresentationError)!Fe {
            ...
            // The modulus's own width and not the type's. `e` is an element
            // of this field, so it cannot need more -- and the ladder spends
            // four squarings on every nibble it is given, so the difference
            // between the two is squarings of leading zeros. For a 2048-bit
            // modulus under a `Modulus(4096)`, which is what an RSA
            // implementation that supports 4096-bit keys uses for all of
            // them, it is exactly half the work.
            //
            // Constant time is unaffected: the length depends on the modulus,
            // which is public, and not on `e`, which is not. `powPublic`
            // already trims this way and goes further, stripping leading zero
            // *bits* -- which this must not do, because that length would
            // depend on the secret.
            //
            // The modulus's bit length rather than its limb count, because a
            // limb holds `t_bits` and not eight: counting limbs asks for more
            // bytes than the buffer has at the top of a type's range, which
            // the differential test caught at a 4096-bit modulus under
            // `Modulus(4096)`.
            const width = math.divCeil(usize, self.bits(), 8) catch unreachable;
            const encoded = buf[0..width];
            e.toBytes(encoded, native_endian) catch unreachable;
            return self.powWithEncodedExponent(x, encoded, native_endian);
        }
